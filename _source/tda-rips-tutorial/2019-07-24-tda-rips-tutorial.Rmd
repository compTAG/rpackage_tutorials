...
Another application is studying coverage in sensor networks. The problem is to look a set of sensors find how much coverage the sensors provide. Vin De Silva and Robert Ghrist studied this problem using Rips complexes since the complex can be determined easily from pairwise communication data. See {% cite ghrist:2007 %}.

Furthermore, Rips complexes can are used in shape reconstruction in specific situations. This means in certain situations, the Rips complex of a point cloud, $$S$$, with threshold, $$r$$, has the same homotopy type as the union of balls centered around each point in $$S$$ with radius $$\frac{r}{2}$$. See {% cite attali:2013 %}. They are nice to use since these complexes do not favor a specific type of alignment of the input.
...
Now let's compute the filtration!

```{r bigfoot_filtration, echo = TRUE, message = TRUE}
r <- 3 #filtration limit

maxdimension <- 2 #allow components, loops, and voids

bigfoot_RipsFilt <- ripsFiltration(bigfoot_sightingsMT, maxdimension, 
                                   r, dist = "euclidean")
```

Lastly, we can visualize the Rips complex for $$r=.15$$. 

```{r bigfootcomplex_show, echo = TRUE, message = FALSE, results = 'hide', eval = FALSE, fig.keep = 'none'}
r <- .15 #filtration limit

maxdimension <- 1 #allow components, loops

bigfoot_RipsFilt <- ripsFiltration(bigfoot_sightingsMT, maxdimension, 
                                   r, dist = "euclidean")
filt <- bigfoot_RipsFilt
...
plot(NULL, type = "n", xlim = xlim, ylim = ylim, main = "Rips Complex",
     xlab = "", ylab = "")

for (idx in seq(along = filt[["cmplx"]])) {
  polygon(data[filt[["cmplx"]][[idx]], , drop = FALSE],
          col = "gray", border = NA, xlim = xlim, ylim = ylim)
}
for (idx in seq(along = filt[["cmplx"]])) {
  polygon(data[filt[["cmplx"]][[idx]], , drop = FALSE],
          col = NULL, xlim = xlim, ylim = ylim)
}
points(data, pch = 16)
dev.off()
```

```{r bigfootcomplex_hide, echo = FALSE, message = FALSE, results = 'hide'}
...
svg('../../assets/tda-rips/bigfoot_complex.svg')
plot(NULL, type = "n", xlim = xlim, ylim = ylim, main = "Rips Complex",
     xlab = "", ylab = "")

for (idx in seq(along = filt[["cmplx"]])) {
...
<center>
<embed width="80%" src="../../assets/tda-rips/bigfoot_complex.svg" type="image/svg+xml" />
</center>

**TODO: figure out why the plot isn't showing up, add some concluding remarks on this example**
