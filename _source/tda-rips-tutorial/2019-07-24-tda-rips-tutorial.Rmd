...
In this tutorial, we work with the Vietoris Rips Filtration, or Rips Filtration for short.
...
We construct a Rips Complex from simplices of varying dimensions that are dimensional
...
A simplicial complex is a nice way of combining simplices. A *face* of a simplex is the convex 
hull of a subset of the vertices in the simplex. A *simplicial complex* is a finite
collection of simplices, $$K$$, such that (1) if $$\sigma \in K$$ and $$\tau\leq \sigma$$, then 
$$\tau \in K$$, and (2) if $$\sigma, \sigma'\in K$$, then $$\sigma\cap \sigma'$$ is either empty 
or a face of both. 
...
An *abstract simplicial complex* is a finite collection of sets, $$A$$, such that if $$a\in A$$ and $$b\subset a$$ implies $$b\in A$$. The elements $$a\in A$$ are the simplices where dim$$(a)=$$card$$(a)-1$$. Here, card$$(a)$$ is the number of points in the set, $$a$$.
...
Before explaining how to construct a Rips complex from a finite set of points, we give a 
preliminary definition. The *diameter* of a set of points is the maximum distance between any 
...
$$S$$. Whenever $$d$$ balls have pairwise intersections, we add a $$d-1$$ dimensional simplex. For this tutorial, we use the standard Euclidean distance (unless stated otherwise) to compute a Rips complex. However, one could use any metric.
...
get different Rips complexes. In most data analysis situations, there is not a best choice for $$r$$ that best describes the data, so why not look at all of them!? Observe if we increase $$r$$ continuously, then we get a family of nested Rips complexes which gives rise to the *Rips filtration*.
...
* Coordinates of galaxies in the universe
...
Persistent Homology (see next tutorial), which is often used to analyze these types of data.

Another application is studying coverage in sensor networks {% cite ghrist:2007 %}. The 
problem is to look a set of sensors and find how much coverage the sensors provide. Vin De 
Silva and Robert Ghrist studied this problem using Rips complexes since the complex can be
determined easily from pairwise communication data. 

Furthermore, Rips complexes can are used in shape reconstruction. This means, under certain assumptions,
the Rips complex of a point cloud, $$S$$, with threshold, $$r$$, has the same homotopy
type as the union of balls centered around each point in $$S$$ with radius $$\frac{r}{2}$$ {% cite
attali:2013 %}. Rips complexes are nice to use since they do not favor a specific type of alignment of the
input.
...
S_RipsFilt <- ripsFiltration(S, maxdimension = 2, maxscale = 5,
                             dist = "euclidean")
```
...
* **values** is a vector where the $$i^{th}$$ entry is the filtration value for when the $$i^{th}$$ simplex appears.
* **increasing**  is a logical variable that states TRUE if the filtration values in the values vector are increasing order and FALSE otherwise. 
* **coordinates** is a matrix representing the coordinates of the vertices. The $$i^{th}$$ row is the coordinate for the $$i^{th}$$ vertex.
...
Our output tells us that we have a two-simplex on vertices $$(0,0),(1,3),(3,2)$$ when $$r=\sqrt{13}$$.
...
If we want to see an animation of the Rips filtration, we can run the following code with a plot function. This function has arguments:
* **filt** is the Rips filtration object
* **animate** is a logical varibale stating TRUE if you want an animation of the filatration or FALSE if you do not want an animation 
* **thresh** if "animate = FALSE" then thresh is the threshold value value for the Rips complex that will be plotted
* **fn_out** if "animate = TRUE" then fn_out is the name of the file for the animation
* **l_out** if "animate = TRUE" then l_out is the number of levels of radii used in the animation between 0 and 1.1 times the max distance between any two points
...
plot(S_RipsFilt, animate = TRUE, fn_out = 'rips_example.gif',
...
Furthermore, we can look at the Rips complex at a threshold of $$\sqrt{13}$$, by adding the argument, "thresh = $$\sqrt{13}$$" and changing "animate = FALSE" to "animate = TRUE". 

```{r toyex_complex, echo = TRUE, message = FALSE, results = 'hide', eval = FALSE, fig.keep = 'none'}

plot(S_RipsFilt, thresh = sqrt(13), animate = FALSE)

```

<center>
<embed width="80%" src="../../assets/tda-rips/toycomplex.svg" type="image/svg+xml" />
</center>
...
When we use the ripsFiltration function, we write dist_matrix as the first argument instead of $$S$$.
Additionally, the fourth argument will change to dist = "arbitrary" to indicate that we are giving our own
distance metric.
...
Hence VR$$(S, \sqrt{13})$$ with the Manhattan distance is a simplicial complex with two edges, while with the Euclidean distance is a simplicial complex with four edges that join to form a cycle.
...
bigfoot_sightingsMT <- cbind(longitude, latitude)
...
Now, let's compute the filtration and visualize it with an animation. In order to speed up the runtime, we will set maxscale = 2.5 and maxdimension = 3. 

```{r bigfootfiltration, echo = TRUE, message = FALSE, results = 'hide', eval = FALSE, fig.keep = 'none'}

BigfootRipsFilt <- ripsFiltration(bigfoot_sightingsMT, maxdimension = 3,
                                  maxscale = 1.5, dist = "euclidean")

class(BigfootRipsFilt) <- c(class(BigfootRipsFilt), "rips_filt")

plot(BigfootRipsFilt, animate = TRUE, fn_out = 'bigfoot.gif',
     lout = 120, interval = 0.3)
...
Furthermore, we can look at the Rips complex at a threshold of one.

```{r bigfootcomplex, echo = TRUE, message = FALSE, results = 'hide', eval = FALSE, fig.keep = 'none'}

plot(BigfootRipsFilt, thresh = 1, animate = FALSE)
...
As we watch the animation, we see that even at low filtration values, there are lots of simplices near the GPS coordinates of Missoula, MT which corresponds to several bigfoot sightings in that area. Furthermore, at a filtration value of one, we see that the area surrounding Helena, MT is a deadzone. If we assume that Bigfoot does not travel radially outwards more than one latitude/longitude unit, then this suggests that hiking in Helena could help avoid Bigfoot encounters. Lastly, there have not been many sightings in Eastern MT, so perhaps Bigfoot does not roam around in that part of the state. We have not fully solved the Bigfoot mystery but the Rips complex and Rips filtration helps us get one step closer!
