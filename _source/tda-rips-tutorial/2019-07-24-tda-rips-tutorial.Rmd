...
simplicial complexes, filtrations, and the Rips complex. *Computation Topology: An Introduction* 
provides more details on the theory. {% cite edelsbrunner:2010 %}

### Triangle Appreciation

We construct a Rips Complex from simplices of varying dimensions that are 
generalizations of triangles of varying dimensions. 
...
$$S$$. Whenever $$d$$ balls have pairwise intersections, we add a $$d-1$$ dimensional simplex. 
For this tutorial, we use the standard Euclidean distance (unless stated otherwise) to compute a Rips
...
intersect. The Rips complex is a generalization of contact graphs since Rips complexes can have more 
than just vertices and edges. 
...
Another application is studying coverage in sensor networks. The problem is to look a set 
...
Furthermore, Rips complexes can are used in shape reconstruction. Dominque Attali, Andr`{e} Lieutier, 
...
* **thresh**  is the threshold value value for the Rips complex that will be plotted if "animate = FALSE".
...
for(j in 1:length(S[, 1])) {
  draw.circle(x = S[j, 1], y = S[j, 2], radius = sqrt(13) / 2,
              col = rgb(0.1, 0.6, 0.1, 0.05))
}
for (idx in seq(along = RipsFiltManhattan[["cmplx"]][RipsFiltManhattan$values < sqrt(13)])) {
